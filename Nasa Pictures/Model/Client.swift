...
class OTMClient : NSObject {
    ...
    func getData(sonda: String, starterDate: Date, result: @escaping (_ result: Result?, _ error: NSError?) -> Void) -> URLSessionDataTask {
        
        let request = NSMutableURLRequest(url: URL(string: "https://api.nasa.gov/mars-photos/api/v1/rovers/" + sonda + "/photos?earth_date=" + getYesterdayDateString(starterDate: starterDate) + "&api_key=U7YRh9sYiU7BH9xyg06bcZEYxeIJCNuIUrIMbYV5")!)
        ...
        let task = session.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            do {
                let gardens = try decoder.decode(Result.self, from: data)
                result(gardens, nil)
            } catch {
